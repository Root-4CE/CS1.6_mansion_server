...
ARG SERVER_NAME="NORTH MERCENARY CS_1.6_server"
...
COPY cs_mansion.bsp /hlds/cstrike/maps/cs_mansion.bsp
COPY cs_mansion.res /hlds/cstrike/maps/cs_mansion.res
COPY cs_mansion.txt /hlds/cstrike/maps/cs_mansion.txt
...
RUN echo "// enable fast download - sv_downloadurl have to start with 'http', end with 'cstrike/', i.e. 'http://10.20.30.40/cstrike/'  " >> /hlds/cstrike/server.cfg && \
    ...
    echo "mp_timelimit 0" >> /hlds/cstrike/server.cfg \
    echo "mp_maxrounds 0" >> /hlds/cstrike/server.cfg
